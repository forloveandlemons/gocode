...
type NewStockHandlerResult struct {
	fx.Out

	Handler   http.Handler `name:"stockHandler"`
}
func NewStockHandler(logger *log.Logger) (NewStockHandlerResult, error) {
	return NewStockHandlerResult{
		Handler: http.HandlerFunc(func(resWriter http.ResponseWriter, req *http.Request) {
			if e := req.ParseForm(); e != nil {
				logger.Print(e)
			}
			stock1 := req.Form.Get("stock1")
			stock2 := req.Form.Get("stock2")
			ds := req.Form.Get("ds")
			...
			queryStr := fmt.Sprintf(
				"SELECT correlation FROM bobo_stock_analysis.corrrelation_result " +
					"WHERE stock1 = '%s' AND stock2 = '%s' AND ds = '%s' AND eventtime >= 1579480230000 AND eventtime < 1579480270000" +
					" LIMIT 1", stock1, stock2, ds)
			logger.Print(queryStr)

			if err := session.Query(queryStr).Scan(&correlation); err != nil {
				logger.Print(err)
			}
			logger.Print(correlation)
			res := fmt.Sprintf("Correlation between %s and %s is %v", stock1, stock2, correlation)
			resWriter.Write([] byte(res))
			logger.Print(res)
	})}, nil
}
...
type RegisterParams struct {
	...
	Mux *http.ServeMux
	StockHandler http.Handler `name:"stockHandler"`
	DefaultHandler http.Handler
}

func Register(params RegisterParams) {
	params.Mux.Handle("/correlation", params.StockHandler)
}
